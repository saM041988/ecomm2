...
const products_reducer = (state, action) => {
    ...
    if (action.type === GET_PRODUCTS_SUCCESSS) {
    ...
    if (action.type === GET_SINGLE_PRODUCTS_SUCCESS) {
        return (
            {
                ...state, single_product_loading:false, single_product:action.payload
            }
        )
    }
    ...
    throw new Error (`No Matching "${action.type}" - action.state`)
}
